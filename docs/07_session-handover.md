...
改札内施設の回遊・滞在・送客を可視化し、**JR東日本スタートアッププログラム**への応募実証実験として機能させること。

- 現フェーズ: **応募前**（実証実験 最小実装の完成が最優先）
...
### ⚠️ kv.keys() による期間集計の限界（本番運用前に対応）

`/api/events/summary?range=7d|30d` の実装では、`kv.keys("event_count:*")` で全件取得してから日付フィルタする方式を採用している。

**実証実験段階では許容**（データ量が少ないため）。ただし本番運用・大量ログ化した場合、以下の問題が生じる:

- `kv.keys()` は Redis の `KEYS *` に相当し、件数が多いとパフォーマンス劣化する
- 期間内の全日付分の `kv.get()` 並列コールが増大する

**将来の改善候補:**
- 日付別インデックスキー: `event_index:{date}` に当日の `eventName` → count を JSON で1キーにまとめる
- 集計キャッシュ: `events:daily_summary:{date}` に日次集計済み結果を保存し、期間集計はキャッシュ合算に限定する
- Vercel KV の `HSCAN` / `HGETALL` を使ったハッシュ型への移行を検討
