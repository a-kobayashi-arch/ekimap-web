...
**修正ファイル:** `data/akabane.json`

**内容:**  
赤羽駅 62 件の seating・outlet フィールドを全件設定。

| 分類 | 件数 | seating | outlet |
|------|------|---------|--------|
| 飲食店 | 14 件 | `yes`（既存値保持） | `unknown` |
| 物販・コンビニ・雑貨等 | 46 件 | `no` | `none` |
| 期間限定（isTemporary） | 2 件 | `no` | `none` |

Node スクリプトで一括変換。tsc / build ✅。

### v0.15.6 — 応募前フェーズ最重要ルール追加（2026-05-06）

**修正ファイル:** `CLAUDE.md`, `docs/03_development-rules.md`

**内容:**  
JR東日本スタートアップ応募前フェーズの優先事項（5 項目）と禁止事項（12 項目）を  
`CLAUDE.md` 冒頭・`docs/03_development-rules.md` 冒頭に追記。

### v0.15.5 — サブディレクトリ CLAUDE.md 追加（2026-04-27）

**修正ファイル:** `app/CLAUDE.md`, `app/api/CLAUDE.md`, `data/CLAUDE.md`, `docs/CLAUDE.md`

### v0.15.4 — docs/ 引き継ぎドキュメント整備（2026-04-27）

**内容:** docs/ 配下に詳細ドキュメント 7 件を作成・CLAUDE.md を要点化。
...
| タグ | 日付 | 内容 |
|------|------|------|
| v0.15.7 | 2026-05-06 | 赤羽駅 seating/outlet 一括設定 |
| v0.15.6 | 2026-05-06 | 応募前フェーズ最重要ルール追加 |
| v0.15.5 | 2026-04-27 | サブディレクトリ CLAUDE.md 追加 |
| v0.15.4 | 2026-04-27 | docs/ 引き継ぎドキュメント整備 |
